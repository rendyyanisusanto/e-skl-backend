...
const processImport = async (filePath, fileName, graduationPeriodId, classId, majorId, userId) => {
  ...
  for (const { rowIndex, values } of rows) {
    const [, nis, nisn, name, gender, birth_place, birth_date, parent_name, address, phone] = values;
    ...
    try {
      let studentClassId = classId || null;
      let studentMajorId = majorId || null;

      const existingStudent = await Student.findOne({ where: { graduationPeriodId, nisn: String(nisn).trim() } });

      let student;
      if (existingStudent) {
        await existingStudent.update({ nis: nis ? String(nis).trim() : existingStudent.nis, name: String(name).trim(), gender: gender || null, birthPlace: birth_place || null, birthDate: birth_date || null, classId: studentClassId, majorId: studentMajorId, parentName: parent_name || null, address: address || null, phone: phone ? String(phone).trim() : null });
        student = existingStudent;
      } else {
        const accessToken = crypto.randomBytes(32).toString('hex');
        student = await Student.create({ graduationPeriodId, classId: studentClassId, majorId: studentMajorId, nis: nis ? String(nis).trim() : null, nisn: String(nisn).trim(), name: String(name).trim(), gender: gender || null, birthPlace: birth_place || null, birthDate: birth_date || null, parentName: parent_name || null, address: address || null, phone: phone ? String(phone).trim() : null, accessToken });
